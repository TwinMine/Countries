...
const LastChoosenPokemon = () => {
  ...
  return (
    <div className="last-pokemon">
      ...
      <div>
        {lastPokemon.length >= 1 ? (
          lastPokemon.map((item, key) => (
            <button
              disabled={
                cooldown ||
                item === pokemonData.species.name ||
                item === pokemonData.name
              }
              style={{
                background:
                  cooldown ||
              ...
              onClick={() => handlePokemonChange(item)}
              key={key}
            >
